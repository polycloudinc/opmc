...
- **Manifest and source layout:** APM requires `apm.yml` and `.apm/` to be siblings. Both must live together — either at the repo root (consumer ref: `polycloudinc/opmc`) or inside `polycloud-opmc-skills-main/` (consumer ref: `polycloudinc/opmc/polycloud-opmc-skills-main`). Final placement TBD based on preferred consumer reference.
