...
import { findOrgFile } from '../utils/discovery';
import { readYaml, writeYaml } from '../utils/yaml';

export async function orgSet(argv: { symbol?: string; name?: string }): Promise<void> {
  ...
  const foundPath = findOrgFile(cwd);
  ...
  if (foundPath) {
    ...
    const newPath = path.join(cwd, 'org.yaml');
